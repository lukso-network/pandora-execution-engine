...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool) *worker {
	...
	if nil != chainConfig.PandoraConfig && len(chainConfig.PandoraConfig.ConsensusInfo) > 0 {
		recommit = chainConfig.PandoraConfig.ConsensusInfo[0].SlotTimeDuration / 3 * time.Second
		...
		worker.skipSealHook = func(t *task) (shouldSkip bool) {
			...
			expectedInterval := chainConfig.PandoraConfig.ConsensusInfo[0].SlotTimeDuration / 3 * time.Second
