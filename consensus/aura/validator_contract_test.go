...
func init() {
	...
	validator1 := crypto.PubkeyToAddress(validatorKey1.PublicKey)
	validatorKey2, _ := crypto.GenerateKey()
	validator2 := crypto.PubkeyToAddress(validatorKey2.PublicKey)
	initialValidators = append(initialValidators, validator1, validator2)

	var (
		db     = rawdb.NewMemoryDatabase()
		engine = New(params.AuraProtocolChanges.Aura, db)
	)
	...
	genspec := &core.Genesis{
		...
		Alloc: map[common.Address]core.GenesisAccount{
			...
			validator1:   {Balance: big.NewInt(params.Ether)},
			validator2:   {Balance: big.NewInt(params.Ether)},
		},
	}
	copy(genspec.ExtraData[extraVanity:], deployerAddr[:])
	genesis := genspec.MustCommit(db)

	// Generate a batch of blocks, each properly signed
	chain, _ := core.NewBlockChain(db, nil, params.AuraProtocolChanges, engine, vm.Config{}, nil, nil)
	defer chain.Stop()
	core.GenerateChain(params.AuraProtocolChanges, genesis, engine, db, 3, func(i int, block *core.BlockGen) { })

	sim := backends.NewSimulatedBackendWithChain(chain, db, params.AuraProtocolChanges)
	defer sim.Close()

	parsed, _ := abi.JSON(strings.NewReader(ownedSetAbi))
	initialAddr := []common.Address{validator1, validator2}
	contractAddr, _, _, _ := bind.DeployContract(opts, parsed, common.FromHex(ownedSetBin), sim, initialAddr)
	sim.Commit()
...
func TestValidatorContract_GetValidatorList(t *testing.T) {
	validatorSetContract, err := NewValidatorSetWithSimBackend(contractAddress, simulatedBackend)
	if err != nil {
		t.Errorf("could not deploy contract: %v error: %v", contractAddress, err)
	}
